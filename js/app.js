...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================
    // Cash Drawer Functionality (WebUSB)
    // ============================================
    let cashDrawerDevice = null;

    async function openCashDrawer() {
        const drawerCmd = new Uint8Array([0x1B, 0x70, 0x00, 0x19, 0xFA]); // ESC p command

        // If we already have a connected device, use it
        if (cashDrawerDevice && cashDrawerDevice.opened) {
            try {
                ...
                console.log('Cached device failed, reconnecting...', e);
        ...
        if (!('usb' in navigator)) {
            showNotification('⚠️ Tu navegador no soporta WebUSB. Usa Chrome o Edge.', 'error');
            return false;
        }

        try {
            // Request device (user must select)
            ...
            const interfaceNumber = cashDrawerDevice.configuration.interfaces[0].interfaceNumber;
            await cashDrawerDevice.claimInterface(interfaceNumber);
            ...
            if (endpoint) {
                await cashDrawerDevice.transferOut(endpoint.endpointNumber, drawerCmd);
                showNotification('✅ Caja abierta (impresora conectada)');
                return true;
            } else {
                showNotification('⚠️ No se encontró endpoint de salida', 'error');
                return false;
            }
        } catch (error) {
            if (error.name === 'NotFoundError') {
                showNotification('❌ No seleccionaste ningún dispositivo', 'error');
            } else {
